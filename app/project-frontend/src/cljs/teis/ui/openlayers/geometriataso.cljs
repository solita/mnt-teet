(ns teis.ui.openlayers.geometriataso
  "Geometriataso, joka muodostaa tason vektorista mäppejä."
  (:require [teis.ui.openlayers.featuret :as featuret]
            [teis.ui.openlayers.taso :as taso :refer [Taso]]
...
(defn- luo-feature [geom]
  (try
    ...
    (catch js/Error e
      (log/warn (pr-str e))
      (log/warn (pr-str "luo-feature palautti virheen, geom: " geom))
      nil)))
...
(defn aseta-feature-geometria! [feature geometria]
  (.set feature "teis-geometria" geometria))
  ...
  (let [create? (nil? geometry-layer)
        geometry-layer (if create?
                         (doto (create-geometry-layer
                                (taso/opacity items))
                           (.setZIndex (or (:z-index (meta items)) 10)))
                         (doto
                           geometry-layer
                           (.setZIndex (or (:z-index (meta items)) 10))))
