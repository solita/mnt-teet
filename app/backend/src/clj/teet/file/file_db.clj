...
(defn files-by-project-and-sequence-number [db user project-id sequence-number]
  (file-listing
   db user
   (mapv first
         (d/q '[:find ?f
...
(defn file-count-by-project-and-sequence-number
  ...
  (count (files-by-project-and-sequence-number db user project-id sequence-number)))
