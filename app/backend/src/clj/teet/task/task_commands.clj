...
(defn review-notification-tx
  ...
  (if-let [to (case role
                ...
                :ta-manager (activity-db/activity-manager db (activity-db/task-activity-id db task-id)))]
    (notification-db/notification-tx
      db
      {:from from
       :to to
  ...
  {:doc "Submit task results for review."
   ...
   :transact (into
               [{:db/id task-id
                ...
                (review-notification-tx db user :ta-manager task-id :notification.type/task-waiting-for-review task-id)]
  ...
  {:doc "Submit task part for review and approve/reject."
   ...
   :transact (into
               [{:db/id taskpart-id
                ...
                (review-notification-tx db user :ta-manager taskpart-id :notification.type/task-part-waiting-for-review task-id)]
