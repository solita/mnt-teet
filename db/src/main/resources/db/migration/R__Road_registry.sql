...
BEGIN
  ...
  FOR r IN SELECT g.stee, g.algus, g.lopp, g.teeosa, g.pikkus, St_LineMerge(g.mgeom) AS line
  ...
  LOOP
    IF startm > r.algus AND endm < r.lopp THEN
      ...
      geom := ST_LineSubstring(r.line, startm/r.pikkus, endm/r.pikkus);
    ELSIF startm > r.algus THEN
      ...
      geom := ST_LineSubstring(r.line, startm/r.pikkus, 1.0);
    ELSIF endm < r.lopp THEN
      -- Road ends in this segment
      RAISE NOTICE '% - % ends in this segment: % - % => %', startm, endm, r.algus, r.lopp, endm/r.lopp;
      geom := ST_LineMerge(ST_Collect(geom, ST_LineSubstring(r.line, 0.0, endm/r.lopp)));
    ELSE
