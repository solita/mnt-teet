describe("Cost items, totals and materials views", function() {
  ...
  context("Cost items", function() {
    ...
    it("new cost item can be created", function() {
      ...
      // Check we're redirected to the cost item view, store cost item oid
    ...
    it("component can be added to the cost item", function() {
      ...
      cy.get("div[data-form-attribute=':culvertpipe/culvertpipediameter'] input").type("10")
      cy.get("div[data-form-attribute=':component/quantity'] input").type("20")
      cy.get("select[id='links-type-select:common/status']").select("Planned")
      cy.get("div[data-form-attribute=':culvertpipe/culvertpipelenght']").type("20")

      // Submit
      cy.get("button[type=submit]").click()

      // Check we're redirected to the cost item view, store component oid
